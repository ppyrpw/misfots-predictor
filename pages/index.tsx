...
function stageClass(stage: string) {
  return { Champion: 's-champion', 'Runner-Up': 's-runner-up', 'Semi-Final': 's-semi', 'Quarter-Final': 's-quarter', 'Round of 16': 's-r16', 'Round of 32': 's-r32', 'Group Stage': 's-group' }[stage] ?? 's-group'
}
...
// League page
// ─────────────────────────────────────────────────────────────────────────────
function LeaguePage({ onNavigate }: { onNavigate: (p: Page) => void }) {
  const { user } = useAuth()
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        ...
        {user
          ? <button className="btn-secondary" onClick={() => onNavigate('predict')}>Edit my picks →</button>
              ...
              <tbody>
                {table.map((row, i) => (
                  <tr key={row.email} style={{ background: user?.email === row.email ? '#f5f4f1' : undefined }}>
                    <td style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 500, color: i === 0 ? '#b45309' : i === 1 ? '#6b7280' : i === 2 ? '#92400e' : 'var(--text-2)' }}>
                      {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : i + 1}
                    </td>
...
// Predictions page
// ─────────────────────────────────────────────────────────────────────────────
function PredictPage({ onNavigate }: { onNavigate: (p: Page) => void }) {
  const { user } = useAuth()
  ...
  const handleChange = (pos: number, val: string) => {
    setPicks(prev => {
      const next = { ...prev }
      if (val) next[pos] = val
  ...
  const SlotSelect = ({ pos }: { pos: number }) => {
    const val = picks[pos] ?? ''
    const usedElsewhere = new Set(
      Object.entries(picks).filter(([k]) => parseInt(k) !== pos).map(([, v]) => v)
    )
    return (
      <div className="pred-slot">
        ...
        >
          ...
          {sortedTeams.map(t => (
            <option key={t.name} value={t.name} disabled={usedElsewhere.has(t.name) && t.name !== val}>
              {t.flag} {t.name}
            </option>
          ))}
        </select>
    ...
    <>
      ...
      <div className="pred-section">
        ...
        <div className="pred-grid">
          {topSlots.map(p => <SlotSelect key={p} pos={p} />)}
        </div>
      </div>
      ...
      <div className="pred-section">
        ...
        <div className="pred-grid">
          {botSlots.map(p => <SlotSelect key={p} pos={p} />)}
        </div>
      </div>
    ...
    <div className="auth-wrap">
      <div className="auth-card">
        ...
        <p style={{ color: 'var(--text-2)', fontSize: 14, marginBottom: 24 }}>
          {mode === 'login' ? 'Sign in to view and update your predictions.' : 'Create your profile to start predicting.'}
        </p>
        ...
        <div className="field"><label>Password</label><input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder={mode === 'login' ? 'Your password' : 'Choose a password'} onKeyDown={e => e.key === 'Enter' && submit()} /></div>
...
function ProfilePage({ onNavigate }: { onNavigate: (p: Page) => void }) {
  ...
  const [stats, setStats] = useState<{ score: number; filled: number; rank: number; total: number } | null>(null)

  useEffect(() => {
    if (!user) return
    Promise.all([fetch('/api/league').then(r => r.json()), fetch('/api/standings').then(r => r.json())])
      .then(([league, standings]) => {
        const table: LeagueRow[] = league.table ?? []
        const ranked = rankStandings(standings.standings ?? [])
